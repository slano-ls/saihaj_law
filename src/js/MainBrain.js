...
import BrainGUI from "./services/gui";
...
class MainBrain extends AbstractApplication {
  constructor() {
    try {
      ...
      // Start the intro after a delay
      setTimeout(() => {
        this.startIntro();
      }, 1000);
      
      console.log("Brain visualization initialized successfully");
    } catch (error) {
      console.error("Error initializing brain visualization:", error);
    }
  }
  ...
  addBrain() {
    ...
    this.scene.add(this.loaders.BRAIN_MODEL);
  }
    ...
    keys.map((m) => {
      if (mesh.name.includes(m)) {
        if (memories[m].length) {
          ...
          memories[m] = [
            BufferGeometryUtils.mergeBufferGeometries(memories[m]),
          ];
          return memories;
        }
  ...
  addParticlesSystem() {
    if (!this.endPointsCollections) {
      console.error("Brain model not loaded properly");
      return;
    }
